...
outfile = R.TFile("JetEleFakeRate_%s.root"%(wps), "RECREATE")
neta = 1
...
for ieta in etas :
    ...
    for ipt in pts :
        ...
        for bin in bins :
            ...
            cb.AddObservations(['*'], ['JetEleFR'], ['2018'], ['lle'], categories['lle']) # adding observed data
            cb.AddProcesses(   ['*'], ['JetEleFR'], ['2018'], ['lle'], backgrounds, categories['lle'], False) # adding backgrounds
            cb.AddProcesses(   ['*'], ['JetEleFR'], ['2018'], ['lle'], signals, categories['lle'], True) # adding signals
            ...
            filepath = os.path.join("/nfs/dust/cms/user/ywen/AZh/workSpacev9/FRworkspace/shapes/pfmt_3_JetToEleFR_%s_%s_%s_%s.root")%(wps,ipt,ieta,bin)
...
canvas.SaveAs("JetEleFR_%s_Prefit.png"%(wps))
canvas2.cd()
h2_FR_postfit.Draw("COLZ L")
canvas2.SaveAs("JetEleFR_%s_Postfit.png"%(wps))
outfile.Close()
