...
outfile = R.TFile("JetTauFakeRate_%s_%s.root"%(wps,era), "RECREATE")
ndm = 1
...
for idm in dms :
    ...
    for ipt in pts :
        ...
        for bin in bins :
            ...
            cb.AddObservations(['*'], ['JetTauFR'], ['2016'], ['llt'], categories['llt']) # adding observed data
            cb.AddProcesses(   ['*'], ['JetTauFR'], ['2016'], ['llt'], signals, categories['llt'], True) # adding signals
            cb.AddProcesses(   ['*'], ['JetTauFR'], ['2016'], ['llt'], backgrounds, categories['llt'], False) # adding backgrounds
            
            cb.cp().process(mc_backgrounds+signals).AddSyst(cb,'lumi_2016', 'lnN', ch.SystMap()(1.026))
            #cb.cp().process(['fake']).AddSyst(cb, 'Norm_fake', 'lnN', ch.SystMap()(2.0))
            cb.cp().process(['prompt']).AddSyst(cb, 'Norm_prompt', 'lnN', ch.SystMap()(1.1))
            filepath = os.path.join("/nfs/dust/cms/user/ywen/AZh/workSpacev11/FRworkspace/shapes/pfmt_3_JetToTauFR_%s_%s_%s_%s.root")%(wps,ipt,idm,bin)
...
canvas.SaveAs("JetTauFR_%s_Prefit_%s.png"%(wps,era))
canvas2.cd()
h2_FR_postfit.Draw("COLZ L")
canvas2.SaveAs("JetTauFR_%s_Postfit_%s.png"%(wps,era))
outfile.Close()
