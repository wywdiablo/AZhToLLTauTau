...
wps = 'Loose_Iso0p15'
pts = ['10to20','20to30','Gt30']#'30to40','Gt40']# '40to60','Gt60']
etas = ['barrel','endcap']
bins = ['Num','Den']
netabins = 2
etabin = [0,1,2]
...
outfile = R.TFile("JetMuFakeRate_%s_%s.root"%(wps,era), "RECREATE")
neta = 1
...
for ieta in etas :
    ...
    for ipt in pts :
        ...
        for bin in bins :
            ...
            cb.AddObservations(['*'], ['JetMuFR'], ['2016'], ['llm'], categories['llm']) # adding observed data
            cb.AddProcesses(   ['*'], ['JetMuFR'], ['2016'], ['llm'], backgrounds, categories['llm'], False) # adding backgrounds
            cb.AddProcesses(   ['*'], ['JetMuFR'], ['2016'], ['llm'], signals, categories['llm'], True) # adding signal

            cb.cp().process(all).AddSyst(cb,'lumi_2016', 'lnN', ch.SystMap()(1.026))
            ...
            filepath = os.path.join("/nfs/dust/cms/user/ywen/AZh/workSpacev12/FRworkspace/shapes/pfmt_3_JetToMuFR_%s_%s_%s_%s.root")%(wps,ipt,ieta,bin)
...
canvas.SaveAs("JetMuFR_%s_Prefit_%s.png"%(wps,era))
canvas2.cd()
h2_FR_postfit.Draw("COLZ L")
canvas2.SaveAs("JetMuFR_%s_Postfit_%s.png"%(wps,era))
outfile.Close()
