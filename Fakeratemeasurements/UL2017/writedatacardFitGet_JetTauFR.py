...
for idm in dms :
    ...
    for ipt in pts :
        ...
        for bin in bins :
            ...
            cb.AddObservations(['*'], ['JetTauFR'], ['2017'], ['llt'], categories['llt']) # adding observed data
            cb.AddProcesses(   ['*'], ['JetTauFR'], ['2017'], ['llt'], signals, categories['llt'], True) # adding signals
            cb.AddProcesses(   ['*'], ['JetTauFR'], ['2017'], ['llt'], backgrounds, categories['llt'], False) # adding backgrounds
            
            cb.cp().process(mc_backgrounds+signals).AddSyst(cb,'lumi_2017', 'lnN', ch.SystMap()(1.026))
            #cb.cp().process(['fake']).AddSyst(cb, 'Norm_fake', 'lnN', ch.SystMap()(2.0))
            cb.cp().process(['prompt']).AddSyst(cb, 'Norm_prompt', 'lnN', ch.SystMap()(1.1))
            filepath = os.path.join("/nfs/dust/cms/user/ywen/AZh/workSpacev5/FRworkspace/shapes/pfmt_3_JetToTauFR_%s_%s_%s_%s.root")%(wps,ipt,idm,bin)
