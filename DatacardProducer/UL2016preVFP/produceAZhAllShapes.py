    ...
    for iZch in Zchannels :
        for ihch in hchannels :
            ...
            for iuncer in Uncerts :
                for updown in Syst :
                    produceSystShape_cmd = 'root -l -q \'ProduceAZhDataCards_Syst.C("%s","Mass_svc_%sscale%s","CMS_%sshape_2016%s","m^{c}_{ll#tau#tau} [GeV]","Events",true,"%s","%s","%s","%s")\'' %(Era,iuncer,updown,iuncer,updown,iZch,ihch,cut,iCat)
                    #os.system(produceSystShape_cmd)
            #Produce Tau ID Systematic Shapes
            for iTauIDSyst in TauIDSyst :
                for updown in Syst :
                    if iTauIDSyst is 'VSjetSF': systname = 'CMS_eff_t_2016'
                    elif iTauIDSyst is 'VSeSF': systname = 'CMS_fake_e_2016'
                    elif iTauIDSyst is 'VSmuSF': systname = 'CMS_fake_m_2016'
            ...
            for iFakeSyst in FakeSyst :
                for updown in Syst :
                    if iFakeSyst is 'JetToEle' : systname = 'CMS_AZh_fr_JetFakeE_2016'
                    elif iFakeSyst is 'JetToMu' : systname = 'CMS_AZh_fr_JetFakeM_2016'
                    elif iFakeSyst is 'JetToTau' : systname = 'CMS_AZh_fr_JetFakeT_2016'
            ...
            produceBtagSystShapeBtagUp_cmd = 'root -l -q \'ProduceAZhDataCards_Syst_Btag.C("%s","Mass_svc","CMS_DeepFlavBtag_2016","Up","Btag","m^{c}_{ll#tau#tau} [GeV]","Events",true,"%s","%s","%s","%s")\''%(Era,iZch,ihch,BtagUpcut,iCat)
            produceBtagSystShapeBtagDown_cmd = 'root -l -q \'ProduceAZhDataCards_Syst_Btag.C("%s","Mass_svc","CMS_DeepFlavBtag_2016","Down","Btag","m^{c}_{ll#tau#tau} [GeV]","Events",true,"%s","%s","%s","%s")\''%(Era,iZch,ihch,BtagDowncut,iCat)
            produceBtagSystShapeMistagUp_cmd = 'root -l -q \'ProduceAZhDataCards_Syst_Btag.C("%s","Mass_svc","CMS_DeepFlavMistag_2016","Up","MisTag","m^{c}_{ll#tau#tau} [GeV]","Events",true,"%s","%s","%s","%s")\''%(Era,iZch,ihch,MisTagUpcut,iCat)
            produceBtagSystShapeMistagDown_cmd = 'root -l -q \'ProduceAZhDataCards_Syst_Btag.C("%s","Mass_svc","CMS_DeepFlavMistag_2016","Down","MisTag","m^{c}_{ll#tau#tau} [GeV]","Events",true,"%s","%s","%s","%s")\''%(Era,iZch,ihch,MisTagDowncut,iCat)
