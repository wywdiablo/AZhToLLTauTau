                                 ...
                                 TString extraCuts = "")
{
    ...
    double xsec[44] = { //in pico barn (pb)
        ...
        0.114,//HW- H-> WW
        0.180,//HW+ H-> WW
        0.163*0.1061*0.1,//HZJ H -> WW ->2L2Nu, Z->LL
        ...
        mumuCat1234Cuts = "fakeRateWeight_Z1*fakeRateWeight_Z2*fakeRateWeight_1*fakeRateWeight_2*(!(" + mumuLeg1ID + ") && !(" + mumuLeg2ID + ") && !(" + Leg3ID + ") && !(" + Leg4ID + "))";
            ...
            {
                //ee
                std::cout << "EE channel " <<i<<":"+samples_ee_UL2017[i]<<std::endl;
                //TFile * file = new TFile("../NTuples/v7/"+samples[i]+".root");
                TFile * file_ee = new TFile("./NTuples_ee"+finalState+"/"+samples_ee_UL2017[i]+".root");
                ...
                TString histName_ee = samples_ee_UL2017[i] + "_"+varName + "_ee";
                if(UseDataCardBinning)
                ...
                //mumu
                std::cout << "MuMu channel " <<i<<":"+samples_mumu_UL2017[i]<<std::endl;
                TFile * file_mumu = new TFile("./NTuples_mm"+finalState+"/"+samples_mumu_UL2017[i]+".root");
                ...
                TString histName_mumu = samples_mumu_UL2017[i] + "_"+varName+ "_mumu";
                if(UseDataCardBinning)
    ...
    double dyRefXSec[5] = {6077.22,
        ...
        1.137*304.4,
        1.137*111.5,
        1.137*44.05};//2018 values(CP5 values)
    double dyRefEvents_ee[5];
    double dyRefEvents_mumu[5];

    for (int iDY=0; iDY<5; ++iDY) {
        TFile * file_ee = new TFile("./NTuples_ee"+finalState+"/"+dyRefSamples_UL2017[iDY]+".root");
        TH1D * histWeightsH_ee = (TH1D*)file_ee->Get("histWeightsH");
        dyRefEvents_ee[iDY] = histWeightsH_ee->GetSumOfWeights();
        
        TFile * file_mumu = new TFile("./NTuples_mm"+finalState+"/"+dyRefSamples_UL2017[iDY]+".root");
        TH1D * histWeightsH_mumu = (TH1D*)file_mumu->Get("histWeightsH");
    ...
    for (int i=0; i<9; ++i) { // run over samples
        TFile * file_ee = new TFile("./NTuples_ee"+finalState+"/"+dySampleNames_UL2017[i]+".root");
        TTree * tree_ee = (TTree*)file_ee->Get("SynTree");
        double normaliza_ee = dyNorm_ee[i];
        TString histNameZ_ee = dySampleNames_UL2017[i] + "_"+varName+"_Z_ee";
        if(UseDataCardBinning)
        ...
        std::cout << dySampleNames_UL2017[i] << " -> DY Z = " << histZ_ee[i]->GetEntries() << " : " << histZ_ee[i]->GetSumOfWeights() << std::endl;
        
        TFile * file_mumu = new TFile("./NTuples_mm"+finalState+"/"+dySampleNames_UL2017[i]+".root");
        TTree * tree_mumu = (TTree*)file_mumu->Get("SynTree");
        double normaliza_mumu = dyNorm_mumu[i];
        TString histNameZ_mumu = dySampleNames_UL2017[i] + "_"+varName+"_Z_mumu";
        if(UseDataCardBinning)
        ...
        std::cout << dySampleNames_UL2017[i] << " -> DY Z = " << histZ[i]->GetEntries() << " : " << histZ[i]->GetSumOfWeights() << std::endl;
    ...
    TFile * file_data = new TFile("./NTuples_mm"+finalState+"/"+samples_mumu_UL2017[0]+".root");
    TTree * tree_data = (TTree*)file_data->Get("SynTree");
    
    TString histName_dataCat1 = samples_mumu_UL2017[0] + "_Cat1_"+varName;
    TString histName_dataCat2 = samples_mumu_UL2017[0] + "_Cat2_"+varName;
    TString histName_dataCat3 = samples_mumu_UL2017[0] + "_Cat3_"+varName;
    TString histName_dataCat4 = samples_mumu_UL2017[0] + "_Cat4_"+varName;
    TString histName_dataCat12 = samples_mumu_UL2017[0] + "_Cat12_"+varName;
    TString histName_dataCat13 = samples_mumu_UL2017[0] + "_Cat13_"+varName;
    TString histName_dataCat14 = samples_mumu_UL2017[0] + "_Cat14_"+varName;
    TString histName_dataCat23 = samples_mumu_UL2017[0] + "_Cat23_"+varName;
    TString histName_dataCat24 = samples_mumu_UL2017[0] + "_Cat24_"+varName;
    TString histName_dataCat34 = samples_mumu_UL2017[0] + "_Cat34_"+varName;
    TString histName_dataCat123 = samples_mumu_UL2017[0] + "_Cat123_"+varName;
    TString histName_dataCat124 = samples_mumu_UL2017[0] + "_Cat124_"+varName;
    TString histName_dataCat234 = samples_mumu_UL2017[0] + "_Cat234_"+varName;
    TString histName_dataCat1234 = samples_mumu_UL2017[0] + "_Cat1234_"+varName;
    if(UseDataCardBinning)
    {
    ...
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat1 = " << hist_DR[0]->GetEntries() << " : " << hist_DR[0]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat2 = " << hist_DR[1]->GetEntries() << " : " << hist_DR[1]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat3 = " << hist_DR[2]->GetEntries() << " : " << hist_DR[2]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat4 = " << hist_DR[3]->GetEntries() << " : " << hist_DR[3]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat12 = " << hist_DR[4]->GetEntries() << " : " << hist_DR[4]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat13 = " << hist_DR[5]->GetEntries() << " : " << hist_DR[5]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat14 = " << hist_DR[6]->GetEntries() << " : " << hist_DR[6]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat23 = " << hist_DR[7]->GetEntries() << " : " << hist_DR[7]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat24 = " << hist_DR[8]->GetEntries() << " : " << hist_DR[8]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat34 = " << hist_DR[9]->GetEntries() << " : " << hist_DR[9]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat123 = " << hist_DR[10]->GetEntries() << " : " << hist_DR[10]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat124 = " << hist_DR[11]->GetEntries() << " : " << hist_DR[11]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat234 = " << hist_DR[12]->GetEntries() << " : " << hist_DR[12]->GetSumOfWeights() << std::endl;
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven Cat1234 = " << hist_DR[13]->GetEntries() << " : " << hist_DR[13]->GetSumOfWeights() << std::endl;
    ...
    TFile * file_data_ee = new TFile("./NTuples_ee"+finalState+"/"+samples_ee_UL2017[0]+".root");
    TTree * tree_data_ee = (TTree*)file_data_ee->Get("SynTree");
    
    TString histName_dataCat1_ee = samples_ee_UL2017[0] + "_Cat1_"+varName;
    TString histName_dataCat2_ee = samples_ee_UL2017[0] + "_Cat2_"+varName;
    TString histName_dataCat3_ee = samples_ee_UL2017[0] + "_Cat3_"+varName;
    TString histName_dataCat4_ee = samples_ee_UL2017[0] + "_Cat4_"+varName;
    TString histName_dataCat12_ee = samples_ee_UL2017[0] + "_Cat12_"+varName;
    TString histName_dataCat13_ee = samples_ee_UL2017[0] + "_Cat13_"+varName;
    TString histName_dataCat14_ee = samples_ee_UL2017[0] + "_Cat14_"+varName;
    TString histName_dataCat23_ee = samples_ee_UL2017[0] + "_Cat23_"+varName;
    TString histName_dataCat24_ee = samples_ee_UL2017[0] + "_Cat24_"+varName;
    TString histName_dataCat34_ee = samples_ee_UL2017[0] + "_Cat34_"+varName;
    TString histName_dataCat123_ee = samples_ee_UL2017[0] + "_Cat123_"+varName;
    TString histName_dataCat124_ee = samples_ee_UL2017[0] + "_Cat124_"+varName;
    TString histName_dataCat234_ee = samples_ee_UL2017[0] + "_Cat234_"+varName;
    TString histName_dataCat1234_ee = samples_ee_UL2017[0] + "_Cat1234_"+varName;
    if(UseDataCardBinning)
    {
    ...
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat1 = " << hist_DR_ee[0]->GetEntries() << " : " << hist_DR_ee[0]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat2 = " << hist_DR_ee[1]->GetEntries() << " : " << hist_DR_ee[1]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat3 = " << hist_DR_ee[2]->GetEntries() << " : " << hist_DR_ee[2]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat4 = " << hist_DR_ee[3]->GetEntries() << " : " << hist_DR_ee[3]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat12 = " << hist_DR_ee[4]->GetEntries() << " : " << hist_DR_ee[4]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat13 = " << hist_DR_ee[5]->GetEntries() << " : " << hist_DR_ee[5]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat14 = " << hist_DR_ee[6]->GetEntries() << " : " << hist_DR_ee[6]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat23 = " << hist_DR_ee[7]->GetEntries() << " : " << hist_DR_ee[7]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat24 = " << hist_DR_ee[8]->GetEntries() << " : " << hist_DR_ee[8]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat34 = " << hist_DR_ee[9]->GetEntries() << " : " << hist_DR_ee[9]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat123 = " << hist_DR_ee[10]->GetEntries() << " : " << hist_DR_ee[10]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat124 = " << hist_DR_ee[11]->GetEntries() << " : " << hist_DR_ee[11]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat234 = " << hist_DR_ee[12]->GetEntries() << " : " << hist_DR_ee[12]->GetSumOfWeights() << std::endl;
    std::cout << samples_ee_UL2017[0] << " -> Data Driven Cat1234 = " << hist_DR_ee[13]->GetEntries() << " : " << hist_DR_ee[13]->GetSumOfWeights() << std::endl;
    ...
    TString histName_data_SS = samples_mumu_UL2017[0] + "_SS_"+varName;
    if(UseDataCardBinning)
    ...
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven SS mumu (before Nor.) = " << hist_SS->GetEntries() << " : " << hist_SS->GetSumOfWeights() << std::endl;
    hist_SS->Scale(reducibleYield_mumu/hist_SS->GetSumOfWeights());

    TString histName_data_SS_ee = samples_ee_UL2017[0] + "_SS_"+varName;
    if(UseDataCardBinning)
    ...
    std::cout << samples_ee_UL2017[0] << " -> Data Driven SS ee (before Nor.) = " << hist_SS_ee->GetEntries() << " : " << hist_SS_ee->GetSumOfWeights() << std::endl;
    ...
    TString histName_data_FR = samples_mumu_UL2017[0] + "_FR_"+varName;
    if(UseDataCardBinning)
    ...
    std::cout << samples_mumu_UL2017[0] << " -> Data Driven FR mumu (before Nor.) = " << hist_FR->GetEntries() << " : " << hist_FR->GetSumOfWeights() << std::endl;
    hist_FR->Scale(reducibleYield_mumu/hist_FR->GetSumOfWeights());

    TString histName_data_FR_ee = samples_ee_UL2017[0] + "_FR_"+varName;
    if(UseDataCardBinning)
    ...
    std::cout << samples_ee_UL2017[0] << " -> Data Driven FR ee (before Nor.) = " << hist_FR_ee->GetEntries() << " : " << hist_FR_ee->GetSumOfWeights() << std::endl;
    ...
    else
    {
        ...
        std::cout << "GGAZh(1200) : " << AZh1000->GetSumOfWeights() << ", " << AZh1000->GetEntries() << std::endl;
        ...
        std::cout << "BBAZh(1200) : " << BBAZh1000->GetSumOfWeights() << ", "  << AZh1000->GetEntries()<< std::endl;
        ...
        TLatex * cms = new TLatex(0.20,0.94,"CMS L = 41.5 fb^{-1} at #sqrt{s} = 13 TeV, "+ZfinalStatesTag+finalState);
        ...
        if(checkSS)
            canv1->Print("./figures/"+varName+"_"+ZfinalStatesTag+finalState+"_SS_DataDriven.pdf","Portrait pdf");
        else
            canv1->Print("./figures/"+varName+"_"+ZfinalStatesTag+finalState+"_DataDriven.pdf","Portrait pdf");
