...
int main(int argc, char * argv[]) {
  ...
  TH1D * inputEventsH = new TH1D("inputEventsH","",1,-0.5,0.5);
    ...
    if(SynchExer == true)
    {
        if(HiggsFinalState == "MT")
            myfile.open ("DESYSyn_eemt.txt");
        if(HiggsFinalState == "ET")
            myfile.open ("DESYSyn_eeet.txt");
        if(HiggsFinalState == "EM")
            myfile.open ("DESYSyn_eeem.txt");
        if(HiggsFinalState == "TT")
            myfile.open ("DESYSyn_eett.txt");
  ...
  TFile *inputFile_visPtResolution = new TFile(TString(cmsswBase)+"/src/"+svFitPtResFile.data());
    ...
    //tagEff_B = (TH1F*)fileTagging->Get("/mutau/DeepJet_b_medium_all");
    //tagEff_C = (TH1F*)fileTagging->Get("/mutau/DeepJet_c_medium_all");
    //tagEff_Light = (TH1F*)fileTagging->Get("/mutau/DeepJet_udsg_medium_all");
    ...
    FakeRateTools fakerate_Zlep1(FakeRateFilesZ,true,"e");
    FakeRateTools fakerate_Zlep2(FakeRateFilesZ,true,"e");
  ...
  for (int iF=0; iF<nTotalFiles; ++iF) {
    ...
    for (Long64_t iEntry=0; iEntry<numberOfEntries; iEntry++) { 
      ...
      if (nEvents%10000==0) 
        ...
        if(isDebug)
        {
            if(!((analysisTree.event_nr==2742451)))//input here the event(s) you want to check
            {
                continue;
            }
        ...
        fakeRateWeight_Z1 = 1;
        fakeRateWeightUp_Z1 = 1;
        fakeRateWeightDown_Z1 = 1;
        
        fakeRateWeight_Z2 = 1;
        fakeRateWeightUp_Z2 = 1;
        fakeRateWeightDown_Z2 = 1;
      ...
      for (unsigned int i=0; i<nfilters; ++i)
      {
        ...
        {
            std::cout << "you are not considering one of the final states of Higgs decay or measuring jet to lepton fake rate"<< std::endl;
            exit(-1);
        }
        ...
        //tau selection
        vector<unsigned int> goodTaus; goodTaus.clear(); //identified taus
        ...
        if(isDebug)
        {
            std::cout << "Passing [goodEles >= 2]" << std::endl;
        }
        
        //Here: count ZEE Found
        if (isData)
        ...
        if(HiggsFinalState == "EE")
        {
            ...
            if(idisoEles.size() > 4) continue;
            if(idisoMuons.size() > 0) continue;
        }
        if(HiggsFinalState == "EM")
        {
            if(goodEles.size() < 3) continue;
            if(goodMuons.size() < 1) continue;
            if(idisoEles.size() > 3) continue;
            if(idisoMuons.size() > 1) continue;
            if(!isData)
            {
                if(idisoEles.size() < 3) continue;
                if(idisoMuons.size() < 1) continue;
        ...
        if(HiggsFinalState == "ET")
        {
            ...
            if(isDebug)
            {
                std::cout << "Passing goodEles.size() >= 3" << std::endl;
            }
            if(idisoEles.size() > 3) continue;
            if(isDebug)
            {
                std::cout << "Passing idisoEles.size() <= 3" << std::endl;
            }
            if(idisoMuons.size() > 0) continue;
            if(isDebug)
            {
                std::cout << "Passing idisoMuons.size() == 0" << std::endl;
            }
            if(goodTaus.size() < 1) continue;
            if(isDebug)
            {
                std::cout << "Passing goodTaus.size() >= 1" << std::endl;
            }
            if(!isData)
            {
                if(idisoEles.size() < 3) continue;
                ...
                if(isDebug)
                {
                    std::cout << "Passing idisoTaus.size() >= 1" << std::endl;
                }
            }
        }
        if(HiggsFinalState == "MT")
        ...
        //std::cout << "id electrons:" << idisoEles.size() << std::endl;
        //std::cout << "id muons:" << idisoMuons.size() << std::endl;
        ...
        for(unsigned int ie1=0;ie1<goodEles.size();++ie1)
        {
            ...
            for(unsigned int ie2=ie1+1;ie2<goodEles.size();++ie2)
            {
                ...
                if (q1*q2 > 0) continue;
                float DR_EE = deltaR(analysisTree.electron_eta[indexTemp1],analysisTree.electron_phi[indexTemp1],analysisTree.electron_eta[indexTemp2],analysisTree.electron_phi[indexTemp2]);
                if(DR_EE < 0.3) continue;
                foundZOS = true;
        ...
        if(HiggsFinalState == "EM")
        {
            for(unsigned int ie=0;ie<goodEles.size();++ie)
            {
                ...
                if ((indexTemp1 == indexElectron1)||(indexTemp1 == indexElectron2)) continue;
                float DR_1E = deltaR(analysisTree.electron_eta[indexElectron1],analysisTree.electron_phi[indexElectron1],analysisTree.electron_eta[indexTemp1],analysisTree.electron_phi[indexTemp1]);
                if(DR_1E < 0.3) continue;
                float DR_2E = deltaR(analysisTree.electron_eta[indexElectron2],analysisTree.electron_phi[indexElectron2],analysisTree.electron_eta[indexTemp1],analysisTree.electron_phi[indexTemp1]);
                if(DR_2E < 0.3) continue;
                    for(unsigned int im=0;im<goodMuons.size();++im)
                    {
                        int indexTemp2 = goodMuons[im];
                        float DR_1M = deltaR(analysisTree.electron_eta[indexElectron1],analysisTree.electron_phi[indexElectron1],analysisTree.muon_eta[indexTemp2],analysisTree.muon_phi[indexTemp2]);
                        if(DR_1M < 0.3) continue;
                        float DR_2M = deltaR(analysisTree.electron_eta[indexElectron2],analysisTree.electron_phi[indexElectron2],analysisTree.muon_eta[indexTemp2],analysisTree.muon_phi[indexTemp2]);
        ...
        if(HiggsFinalState =="E")
        {
            for(unsigned int ie=0;ie<goodEles.size();++ie)
            {
                ...
                if ((indexTemp1 == indexElectron1)||(indexTemp1 == indexElectron2))
                    continue;
                float DR_1E1 = deltaR(analysisTree.electron_eta[indexElectron1],analysisTree.electron_phi[indexElectron1],analysisTree.electron_eta[indexTemp1],analysisTree.electron_phi[indexTemp1]);
                float DR_2E1 = deltaR(analysisTree.electron_eta[indexElectron2],analysisTree.electron_phi[indexElectron2],analysisTree.electron_eta[indexTemp1],analysisTree.electron_phi[indexTemp1]);
                if((DR_1E1 < 0.3)||(DR_2E1 < 0.3))
                {
                    continue;
                }
                if(indexLeg3!= -1)
        ...
        if(HiggsFinalState =="MT")
        {
            for(unsigned int im=0;im<goodMuons.size();++im)
            {
                ...
                float DR_1M = deltaR(analysisTree.electron_eta[indexElectron1],analysisTree.electron_phi[indexElectron1],analysisTree.muon_eta[indexTemp1],analysisTree.muon_phi[indexTemp1]);
                if(DR_1M < 0.3) continue;
                float DR_2M = deltaR(analysisTree.electron_eta[indexElectron2],analysisTree.electron_phi[indexElectron2],analysisTree.muon_eta[indexTemp1],analysisTree.muon_phi[indexTemp1]);
                if(DR_2M < 0.3) continue;
                for(unsigned int it=0;it<goodTaus.size();++it)
                {
                    int indexTemp2 = goodTaus[it];
        ...
        if(HiggsFinalState =="M")
        {
            ...
            {
                int indexTemp1 = goodMuons[im];
                float DR_1M1 = deltaR(analysisTree.electron_eta[indexElectron1],analysisTree.electron_phi[indexElectron1],analysisTree.muon_eta[indexTemp1],analysisTree.muon_phi[indexTemp1]);
                float DR_2M1 = deltaR(analysisTree.electron_eta[indexElectron2],analysisTree.electron_phi[indexElectron2],analysisTree.muon_eta[indexTemp1],analysisTree.muon_phi[indexTemp1]);
        ...
        {
            if(!analysisTree.electron_mva_wp90_noIso_Fall17_v2[indexElectron1]) continue;
            if(!analysisTree.electron_mva_wp90_noIso_Fall17_v2[indexElectron2]) continue;
            if(HiggsFinalState == "EM")
            {
        ...
        iso_1 = analysisTree.electron_eaIsolation[indexElectron1]/analysisTree.electron_pt[indexElectron1];
        if(!isData)
        {
            IdSF_1 = (float)SF_eleIdIso->get_ScaleFactor(double(analysisTree.electron_pt[indexElectron1]),double(analysisTree.electron_eta[indexElectron1]));
            if(isSingleElectronTriggerFilterMatched && isElectron1SingleElectronMatched)
                TrigSF_1 = (float)SF_eleTrig->get_ScaleFactor(double(analysisTree.electron_pt[indexElectron1]),double(analysisTree.electron_eta[indexElectron1]));
        }
        ...
        iso_2 = analysisTree.electron_eaIsolation[indexElectron2]/analysisTree.electron_pt[indexElectron2];
        
        if(!isData)
        {
            IdSF_2 = (float)SF_eleIdIso->get_ScaleFactor(double(analysisTree.electron_pt[indexElectron2]),double(analysisTree.electron_eta[indexElectron2]));
            if(isSingleElectronTriggerFilterMatched && isElectron2SingleElectronMatched)
                TrigSF_2 = (float)SF_eleTrig->get_ScaleFactor(double(analysisTree.electron_pt[indexElectron2]),double(analysisTree.electron_eta[indexElectron2]));
        }
        ...
        if(HiggsFinalState == "E")
        {
            ...
            //iso_ea_4 = analysisTree.electron_eaIsolation[indexLeg4];
            if(!isData)
            {
        ...
        /*
        if(!isData)//filter out by signal region isolation selection for MC
        {
            if(iso_ea_1>0.15) continue;
            if(iso_ea_2>0.15) continue;
            if(HiggsFinalState == "EM")
            {
        ...
        if(applySystShift && !isData)
        {
            ...
            if(HiggsFinalState == "EM")
            {
                metx_mscaleUp = metLV.X() - Leg4.X() * MuonScale;
                mety_mscaleUp = metLV.Y() - Leg4.Y() * MuonScale;
                metx_mscaleDown = metLV.X() + Leg4.X() * MuonScale;
                mety_mscaleDown = metLV.Y() + Leg4.Y() * MuonScale;
                
                metx_escaleUp = metLV.X() - (analysisTree.electron_px_energyscale_up[indexElectron1] - analysisTree.electron_px[indexElectron1]) - (analysisTree.electron_px_energyscale_up[indexElectron2] - analysisTree.electron_px[indexElectron2]) - (analysisTree.electron_px_energyscale_up[indexLeg3] - analysisTree.electron_px[indexLeg3]);
                mety_escaleUp = metLV.Y() - (analysisTree.electron_py_energyscale_up[indexElectron1] - analysisTree.electron_py[indexElectron1]) - (analysisTree.electron_py_energyscale_up[indexElectron2] - analysisTree.electron_py[indexElectron2]) - (analysisTree.electron_py_energyscale_up[indexLeg3] - analysisTree.electron_py[indexLeg3]);
                metx_escaleDown = metLV.X() + (analysisTree.electron_px[indexElectron1] - analysisTree.electron_px_energyscale_down[indexElectron1]) + (analysisTree.electron_px[indexElectron2] - analysisTree.electron_px_energyscale_down[indexElectron2]) + (analysisTree.electron_px[indexLeg3] - analysisTree.electron_px_energyscale_down[indexLeg3]);
                mety_escaleDown = metLV.Y() + (analysisTree.electron_py[indexElectron1] - analysisTree.electron_py_energyscale_down[indexElectron1]) + (analysisTree.electron_py[indexElectron2] - analysisTree.electron_py_energyscale_down[indexElectron2]) + (analysisTree.electron_py[indexLeg3] - analysisTree.electron_py_energyscale_down[indexLeg3]);
            }
            ...
            if(HiggsFinalState == "MT")
            {
                metx_mscaleUp = metLV.X() - Leg3.X() * MuonScale;
                mety_mscaleUp = metLV.Y() - Leg3.Y() * MuonScale;
                metx_mscaleDown = metLV.X() + Leg3.X() * MuonScale;
                mety_mscaleDown = metLV.Y() + Leg3.Y() * MuonScale;
                
                metx_escaleUp = metLV.X() - (analysisTree.electron_px_energyscale_up[indexElectron1] - analysisTree.electron_px[indexElectron1]) - (analysisTree.electron_px_energyscale_up[indexElectron2] - analysisTree.electron_px[indexElectron2]);
                mety_escaleUp = metLV.Y() - (analysisTree.electron_py_energyscale_up[indexElectron1] - analysisTree.electron_py[indexElectron1]) - (analysisTree.electron_py_energyscale_up[indexElectron2] - analysisTree.electron_py[indexElectron2]);
                metx_escaleDown = metLV.X() + (analysisTree.electron_px[indexElectron1] - analysisTree.electron_px_energyscale_down[indexElectron1]) + (analysisTree.electron_px[indexElectron2] - analysisTree.electron_px_energyscale_down[indexElectron2]);
                mety_escaleDown = metLV.Y() + (analysisTree.electron_py[indexElectron1] - analysisTree.electron_py_energyscale_down[indexElectron1]) + (analysisTree.electron_py[indexElectron2] - analysisTree.electron_py_energyscale_down[indexElectron2]);
        ...
        TLorentzVector electron1UpLV;
        TLorentzVector electron1DownLV;
        TLorentzVector electron2UpLV;
        TLorentzVector electron2DownLV;
        
        TLorentzVector Leg3UpLV;
        TLorentzVector Leg3DownLV;//if it is a hadronic tau, then it is tau energy scale
        ...
        if(applySystShift && !isData)
        {
            electron1UpLV.SetXYZM(analysisTree.electron_px_energyscale_up[indexElectron1],
                                  analysisTree.electron_py_energyscale_up[indexElectron1],
                                  analysisTree.electron_pz_energyscale_up[indexElectron1],
                                  classic_svFit::electronMass);
            electron1DownLV.SetXYZM(analysisTree.electron_px_energyscale_down[indexElectron1],
                                  analysisTree.electron_py_energyscale_down[indexElectron1],
                                  analysisTree.electron_pz_energyscale_down[indexElectron1],
                                  classic_svFit::electronMass);
            electron2UpLV.SetXYZM(analysisTree.electron_px_energyscale_up[indexElectron2],
                                  analysisTree.electron_py_energyscale_up[indexElectron2],
                                  analysisTree.electron_pz_energyscale_up[indexElectron2],
                                  classic_svFit::electronMass);
            electron2DownLV.SetXYZM(analysisTree.electron_px_energyscale_down[indexElectron2],
                                  analysisTree.electron_py_energyscale_down[indexElectron2],
                                  analysisTree.electron_pz_energyscale_down[indexElectron2],
                                  classic_svFit::electronMass);
        ...
        m2l_1 = (electron1+electron2).M();
        m2l_2 = (electron1+Leg3).M();
        m2l_3 = (electron2+Leg3).M();
        m3l = (electron1+electron2+Leg3).M();
        ...
        //std::cout << "index leg1: " <<indexElectron1<<std::endl;
        //std::cout << "index leg2: " <<indexElectron2<<std::endl;
        //std::cout << "index leg3: " <<indexLeg3<<std::endl;
        //std::cout << "index leg4: " <<indexLeg4<<std::endl;
        //std::cout << "DR12: " << DR_12 <<std::endl;
        //std::cout << "DR13: " << DR_13 <<std::endl;
        //std::cout << "DR14: " << DR_14 <<std::endl;
        //std::cout << "DR23: " << DR_23 <<std::endl;
        //std::cout << "DR24: " << DR_24 <<std::endl;
        //std::cout << "DR34: " << DR_34 <<std::endl;
        ...
        if(ApplySVFit||ApplyFastMTT)
        {
            ...
            if (ApplySVFit)
            {
                ...
                Mass_sv = (electron1+electron2+ditau_sv).M();
                ...
                Mass_svc = (electron1+electron2+ditau_svc).M();
                ...
                if(applySystShift && !isData)
                {
                    if(HiggsFinalState == "EM")
                    {
                        ...
                        Mass_svc_mscaleUp = (electron1+electron2+ditau_svc_mscaleUp).M();
                        ...
                        Mass_svc_mscaleDown = (electron1+electron2+ditau_svc_mscaleDown).M();
                        ...
                        Mass_svc_escaleUp = (electron1UpLV+electron2UpLV+ditau_svc_escaleUp).M();
                        ...
                        Mass_svc_escaleDown = (electron1DownLV+electron2DownLV+ditau_svc_escaleDown).M();
                        ...
                        Mass_svc_unclmetscaleUp = (electron1+electron2+ditau_svc_unclmetscaleUp).M();
                        ...
                        Mass_svc_unclmetscaleDown = (electron1+electron2+ditau_svc_unclmetscaleDown).M();
                        ...
                        Mass_svc_jecscaleUp = (electron1+electron2+ditau_svc_jecscaleUp).M();
                        ...
                        Mass_svc_jecscaleDown = (electron1+electron2+ditau_svc_jecscaleDown).M();
                        ...
                        if(era == "UL2018")
                        {
                            ...
                            Mass_svc_HEM2018scaleUp = (electron1+electron2+ditau_svc_HEM2018scaleUp).M();
                            ...
                            Mass_svc_HEM2018scaleDown = (electron1+electron2+ditau_svc_HEM2018scaleDown).M();
                    ...
                    if(HiggsFinalState == "ET")
                    {
                        ...
                        Mass_svc_tscaleUp = (electron1+electron2+ditau_svc_tscaleUp).M();
                        ...
                        Mass_svc_tscaleDown = (electron1+electron2+ditau_svc_tscaleDown).M();
                        ...
                        Mass_svc_eftscaleUp = (electron1+electron2+ditau_svc_eftscaleUp).M();
                        ...
                        Mass_svc_eftscaleDown = (electron1+electron2+ditau_svc_eftscaleDown).M();
                        ...
                        Mass_svc_mftscaleUp = (electron1+electron2+ditau_svc_mftscaleUp).M();
                        ...
                        Mass_svc_mftscaleDown = (electron1+electron2+ditau_svc_mftscaleDown).M();
                    ...
                    if(HiggsFinalState == "MT")
                    {
                        ...
                        Mass_svc_escaleUp = (electron1UpLV+electron2UpLV+ditau_svc).M();
                        res_svc_escaleUp = (Mass_svc_escaleUp - Mass_gen)/Mass_svc_escaleUp;
                        Mass_svc_escaleDown = (electron1DownLV+electron2DownLV+ditau_svc).M();
                        res_svc_escaleDown = (Mass_svc_escaleDown - Mass_gen)/Mass_svc_escaleDown;
            ...
            if (ApplyFastMTT)
            {
                ...
                Mass_fast = (electron1+electron2+ditau_fast).M();
                ...
                //aFastMTTAlgo_c.runMassConstraint(measuredTauLeptons, measuredMETx, measuredMETy, covMET, 125, 1);
                //aFastMTTAlgo_c.runMassConstraint(measuredTauLeptons, measuredMETx, measuredMETy, covMET, 125.38, 2);
                ...
                Mass_fastc = (electron1+electron2+ditau_fastc).M();
        ...
        {
            float fakerate_Z1 = fakerate_Zlep1.GetEleFakeRate(pt_1,eta_1,true);
            float fakerateUp_Z1 = fakerate_Zlep1.GetEleFakeRateUp(pt_1,eta_1,true,"68");
            float fakerateDown_Z1 = fakerate_Zlep1.GetEleFakeRateDown(pt_1,eta_1,true,"68");
            
            float fakerate_Z2 = fakerate_Zlep2.GetEleFakeRate(pt_2,eta_2,true);
            float fakerateUp_Z2 = fakerate_Zlep2.GetEleFakeRateUp(pt_2,eta_2,true,"68");
            float fakerateDown_Z2 = fakerate_Zlep2.GetEleFakeRateDown(pt_2,eta_2,true,"68");
        ...
        //std::cout << "Electron size:" << goodEles.size() <<std::endl;
        //std::cout << "Muon     size:" << goodMuons.size() <<std::endl;
        //std::cout << "Tauh     size:" << goodTaus.size() <<std::endl;
        //std::cout << "--------------" <<std::endl;
